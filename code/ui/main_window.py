...
class DroneGroundStation(QMainWindow):
    ...
    def init_ui(self):
        ...
        for btn in [self.btn_waypoint, self.btn_param, self.btn_vehicle, self.btn_links, self.btn_link, self.btn_mp]:
    ...
    def execute_flight_command(self, command_name, vehicle_id: str | None = None):
        ...
        confirm_messages = {
            "disarm": "请确认当前已落地或处于安全状态，是否继续执行上锁？",
            "vtol_qland": "将立即触发 VTOL 垂直降落（QLAND），是否继续？",
            "vtol_qrtl": "将触发 VTOL 垂直返航（QRTL），是否继续？",
        }
        if command_name in confirm_titles:
            if not gcs_confirm(
                self,
                confirm_titles[command_name],
                confirm_messages[command_name],
                yes_text="确认执行",
                no_text="取消",
                danger=True,
            ):
        ...
        if command_name == "vtol_takeoff_30m":
            if not self._ensure_vtol_mode(thread, "QGUIDED", fallback_modes={"GUIDED", "QLOITER"}):
                return
        elif command_name == "map_fly_to_waypoint":
    ...
    def _command_display_name(command_name: str) -> str:
        ...
        return mapping.get(str(command_name or ""), str(command_name or "指令"))
    ...
    def _refresh_mp_action_states(self):
        ...
        workbench.set_action_active("vehicle.panel", len(self.vehicle_manager.vehicle_summaries()) > 0)
    ...
    def export_parameters_to_file(self):
        file_path, _ = QFileDialog.getSaveFileName(self, "导出参数", "params.json", "JSON files (*.json)")
        if not file_path:
            return
        try:
            self.parameter_manager.export_to_file(file_path)
            self._show_auto_notice("导出成功", f"参数已导出到 {file_path}")
            self.log_user_action("params_exported_to_file", file=file_path)
        except Exception as exc:
    ...
    def add_waypoint(self, lat, lon):
        ...
        if not (math.isfinite(lat) and math.isfinite(lon) and -90.0 <= lat <= 90.0 and -180.0 <= lon <= 180.0):
            ...
            return
        wp = self._build_waypoint_from_map_click(lat, lon)
        active_vehicle_id = str((self.vehicle_manager.active_vehicle() or {}).get("vehicle_id", "") or "")
    ...
    def _build_waypoint_from_map_click(self, lat: float, lon: float, alt_hint: float = 0.0) -> dict:
        if alt_hint > 0:
