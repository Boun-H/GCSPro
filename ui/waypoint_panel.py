...
class WaypointPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self._plan_constraints: Dict = {}
        self._last_safety_report: Dict = {}
    ...
    def _init_ui(self):
        ...
        self.plan_safety_label = QLabel("规划安全检查：等待任务点/禁飞区数据")
        self.plan_safety_label.setWordWrap(True)
        self.plan_safety_label.setStyleSheet("color: #d7e8fb; font-size: 12px; padding: 4px 8px; background: #101a28; border:1px solid #23384f; border-radius: 4px;")
        content_layout.addWidget(self.plan_safety_label)
    ...
    def _capture_current_vehicle_context(self):
        ...
        self._vehicle_contexts[vehicle_id] = {
            ...
            "plan_constraints": dict(self._plan_constraints or {}),
    ...
    def activate_vehicle_tab(self, vehicle_id: str, emit_signal: bool = False):
        ...
        self._plan_constraints = dict(ctx.get("plan_constraints") or {})
    ...
    def set_plan_constraints(self, constraints: Dict | None, vehicle_id: Optional[str] = None):
        target_id = str(vehicle_id or self._active_vehicle_id or "").strip()
        payload = dict(constraints or {})
        if target_id:
            ctx = self._vehicle_contexts.setdefault(target_id, self._blank_vehicle_context())
            ctx["plan_constraints"] = payload
            if target_id != self._active_vehicle_id and self._active_vehicle_id:
                return
        self._plan_constraints = payload
        self.update_table()
    ...
    def update_table(self):
        ...
        safety_report = analyze_plan_safety(
            home=self._home_wp,
            waypoints=mission_waypoints,
            geofence=(self._plan_constraints or {}).get("geofence"),
            min_clearance_m=float((self._plan_constraints or {}).get("min_clearance_m", 30.0) or 30.0),
        )
        self._last_safety_report = dict(safety_report or {})
        metrics_text = f"任务 {len(mission_waypoints)} 点 | 航程 {total_distance_m / 1000.0:.2f} km | 预计 {eta_seconds / 60.0:.1f} min"
        if safety_report.get("clearance_min_m") is not None:
            metrics_text += f" | 最小离地 {float(safety_report.get('clearance_min_m', 0.0)):.1f}m"
        self.summary_label.setText(f"{summary}\n{metrics_text}")
        messages = list((safety_report or {}).get("messages") or [])
        preview = "\n".join(f"• {message}" for message in messages[:3]) if messages else "• 未发现明显地形/冲突/禁飞区风险"
        level = str((safety_report or {}).get("level", "ok") or "ok")
        tone = {
            "ok": ("#103428", "#1f8b5f", "#c9f7df"),
            "warn": ("#3c2a11", "#c38b22", "#fde7a7"),
            "danger": ("#3d1d26", "#c25565", "#ffd5dc"),
        }.get(level, ("#101a28", "#23384f", "#d7e8fb"))
        bg, border, fg = tone
        self.plan_safety_label.setStyleSheet(
            f"color: {fg}; font-size: 12px; padding: 4px 8px; background: {bg}; border:1px solid {border}; border-radius: 4px;"
        )
        self.plan_safety_label.setText(f"{safety_report.get('summary', '规划检查中')}\n{preview}")
        if self._home_wp:
